 ...
 * @param {{ table?: string, method: 'insert'|'update'|'upsert'|'delete'|'rpc', data?: object, id?: string, match?: object, rpcName?: string, rpcArgs?: object }} op
 */
...
export async function syncQueue(supabaseClient, showToast) {
  ...
  for (const op of ops) {
    try {
      ...
      } else if (op.method === 'rpc') {
        query = supabaseClient.rpc(op.rpcName, op.rpcArgs || {})
