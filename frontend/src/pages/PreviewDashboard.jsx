...
function DemoBadge() {
  return <span className="inline-flex items-center gap-2 rounded-full border border-nk-200 bg-nk-50 px-3 py-1.5 text-xs font-black text-nk-700"><Sparkles className="h-3.5 w-3.5" /> معاينة تجريبية</span>
}
...
export default function PreviewDashboard({ onBack, onLogin }) {
  ...
  return <main className="min-h-screen bg-[#f4f7fb] text-slate-900" dir="rtl">
    ...
    <div className="mx-auto max-w-7xl px-5 py-6 sm:px-8 sm:py-8"><div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between"><div><DemoBadge /><h1 className="mt-4 text-3xl font-black tracking-tight sm:text-4xl">شوف المنصة قبل ما تبدأ</h1><p className="mt-3 max-w-2xl text-sm leading-7 text-slate-500">دي جولة تفاعلية ببيانات تجريبية فقط. افتح كل تبويب وشوف النتيجة التي سيحصل عليها المدرس يوميًا.</p></div><div className="rounded-2xl border border-nk-200 bg-nk-50 px-4 py-3 text-xs font-bold leading-6 text-nk-700">لا يتم حفظ أي شيء<br />ولا توجد بيانات حقيقية هنا.</div></div>
      <div className="mt-7 grid gap-6 lg:grid-cols-[1fr_280px] lg:items-start"><section className="min-w-0"><div className="mb-5 flex gap-2 overflow-x-auto rounded-3xl border border-slate-200 bg-white p-2 shadow-sm">{TABS.map(({ id, label, icon: Icon }) => <button type="button" key={id} onClick={() => setActiveTab(id)} className={`flex min-h-11 shrink-0 items-center gap-2 rounded-2xl px-4 text-sm font-black transition ${activeTab === id ? 'bg-slate-900 text-white shadow-sm' : 'text-slate-500 hover:bg-slate-50'}`}><Icon className="h-4 w-4" />{label}</button>)}</div><div className="mb-5 flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-white text-slate-700 shadow-sm"><ActiveIcon className="h-5 w-5" /></div><div><p className="text-xs font-bold text-slate-400">معاينة حية</p><h2 className="text-xl font-black">{active.label}</h2></div></div>{renderContent()}</section><aside className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm lg:sticky lg:top-24"><div className="flex items-center justify-between"><div className="flex items-center gap-2"><Sparkles className="h-5 w-5 text-nk-600" /><h2 className="font-black">جولة سريعة</h2></div><button type="button" onClick={onBack} aria-label="إغلاق المعاينة" className="rounded-xl p-2 text-slate-400 hover:bg-slate-50"><X className="h-4 w-4" /></button></div><div className="mt-5 rounded-2xl bg-slate-50 p-4"><p className="text-xs font-black text-emerald-700">٠{tourStep + 1} / ٠{TOUR_STEPS.length}</p><h3 className="mt-3 font-black">{TOUR_STEPS[tourStep].title}</h3><p className="mt-2 text-sm leading-6 text-slate-500">{TOUR_STEPS[tourStep].text}</p></div><div className="mt-4 flex gap-2">{TOUR_STEPS.map((step, index) => <button type="button" key={step.title} onClick={() => setTourStep(index)} aria-label={`خطوة ${index + 1}`} className={`h-2 flex-1 rounded-full ${index === tourStep ? 'bg-slate-900' : 'bg-slate-200'}`} />)}</div><div className="mt-5 flex gap-2"><button type="button" disabled={tourStep === 0} onClick={() => setTourStep((step) => Math.max(0, step - 1))} className="flex-1 rounded-2xl border border-slate-200 py-3 text-xs font-black text-slate-600 disabled:opacity-40">السابق</button><button type="button" onClick={() => setTourStep((step) => Math.min(TOUR_STEPS.length - 1, step + 1))} className="flex-1 rounded-2xl bg-slate-900 py-3 text-xs font-black text-white">{tourStep === TOUR_STEPS.length - 1 ? 'تم' : 'التالي'}</button></div></aside></div></div>
