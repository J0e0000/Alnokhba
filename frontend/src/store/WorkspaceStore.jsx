...
export function WorkspaceProvider({ children }) {
  ...
  }, [effectiveTeacherId, refreshAbsenceStreak, refreshTodayGroups, refreshSettings, loadAll])
  ...
  const sessionStudentsFor = useCallback((groupName) => {
    if (!groupName) return []
    return students.filter((s) => s.group_name === groupName).map((s) => ({
      ...s,
  ...
  }, [students, lessonAttendanceByStudent, activeLessonId])
  ...
  const saveSessionContent = async (lessonId, draft) => {
    ...
    if (lesson?.status === 'completed') { showToast(isArabic ? 'الحصة منتهية ولا يمكن تعديلها' : 'This session is completed and cannot be edited', 'error'); return false }
    const payload = {
  ...
  const finishLesson = useCallback(async (lessonId, groupName) => {
    const lesson = lessonSessions.find((l) => l.id === lessonId) || (lessonId ? { id: lessonId } : null)
  ...
  }, [effectiveTeacherId, isArabic, showToast, lessonSessions, students, isOnline, validateLessonState])
  ...
  const markGroupAbsences = useCallback(async (lessonId) => {
    ...
    return marked
  }, [effectiveTeacherId, isOnline, isArabic, showToast])

  const markAllPresent = async (groupName, lessonIdOverride) => {
    const list = students.filter((s) => s.group_name === groupName)
    let count = 0
    for (const s of list) {
      const current = lessonIdOverride ? lessonAttendanceByStudent[s.id]?.status : s.attendance_status
      if (current !== 'حاضر') { await setAttendance(s.id, 'حاضر', lessonIdOverride); count++ }
    }
    if (count > 0) showToast(isArabic ? `تم تسجيل ${count} طالب حاضر` : `Marked ${count} students present`, 'success')
    else showToast(isArabic ? 'الكل مسجل حاضر بالفعل' : 'Everyone is already marked present', 'info')
    return count
  }
  ...
  const bulkAddStudents = async (rows, onProgress) => {
    ...
    pushAction({
      ...
      undoFn: async () => {
        for (const s of data) await supabase.from('students').delete().eq('id', s.id)
  ...
  const saveExam = async ({ title, sections, maxScorePerSection, records, lessonId }) => {
    ...
    setIsSaving(true)
    try {
      if (!isOnline) { showToast(t('no_internet_exam') || 'لا يوجد اتصال — تعذر حفظ الامتحان', 'error'); return false }
      ...
      for (const r of records) {
        const { error: scoreError } = await supabase.from('exam_scores').insert({
      ...
      showToast(t('exam_failed') || 'تعذر حفظ الامتحان', 'error')
      return false
    } finally { setIsSaving(false) }
  }
  ...
  const handleUndo = async () => {
    ...
    refreshUndo()
    if (result.undone) { showToast(`${t('undo_done') || 'تم التراجع عن'} ${result.description}`, 'info'); setCanRedoState(getRedoCount() > 0); loadAll() }
    else showToast(t('nothing_to_undo') || 'لا يوجد ما يتم التراجع عنه', 'info')
  }
  const handleRedo = async () => {
    const result = await redoLast()
    refreshUndo()
    if (result.redone) { showToast(`${t('redo_done') || 'تم الإعادة'} ${result.description}`, 'info'); setCanRedoState(getRedoCount() > 0); loadAll() }
    else showToast(t('nothing_to_redo') || 'لا يوجد ما يتم إعادته', 'info')
  }
  const handleHistoryRestore = async (id) => {
    const result = await undoById(id)
    if (result.undone) { showToast(`${t('undo_done') || 'تم التراجع عن'} ${result.description}`, 'info'); loadAll() }
    else showToast(t('undo_failed') || 'تعذر التراجع', 'error')
  }
  ...
  const addGroup = async ({ name, stage, day, time }) => {
    ...
    const { data: scheduleRow, error: scheduleError } = await supabase.from('group_schedule')
      .insert({ teacher_id: effectiveTeacherId, group_name: name, weekday: Number(day), lesson_time: time })
  ...
  const updateGroup = async (groupName, { stage, day, time }) => {
    ...
    try {
      ...
      const { error } = await supabase.from('group_schedule').upsert(
        { teacher_id: effectiveTeacherId, group_name: groupName, weekday: Number(day), lesson_time: time },
  ...
  const value = {
    ...
    // save state
    savingIds, savedIds, isSaving, saveStatus, lastSavedAt, isOnline,
  ...
  if (typeof window !== 'undefined') window.__NOKHBA_WS = value
