...
export default function ExamModal({ open, onClose, students, groups = [], onSave, isSaving: externalSaving, attendanceByStudent = {} }) {
  ...
  // (Round 8) حماية الغايبين: الطالب الغايب مش بيظهر في قائمة الرصد الافتراضية
  // ولا بياخد درجة عادية — لازم يتعلّم حاضر الأول (المسار المصرح بيه).
  const presentStudents = useMemo(() => examStudents.filter((s) => attendanceByStudent[s.id] !== 'غائب'), [examStudents, attendanceByStudent])
  const absentStudents = useMemo(() => examStudents.filter((s) => attendanceByStudent[s.id] === 'غائب'), [examStudents, attendanceByStudent])
  ...
  const handleKeyDown = (e, row, col) => { if (e.key !== 'Enter') return; e.preventDefault(); if (col + 1 < sections.length) focusCell(row, col + 1); else if (row + 1 < presentStudents.length) focusCell(row + 1, 0) }
  ...
  const startGrading = () => {
    ...
    setSections(secs)
    const initial = {}
    presentStudents.forEach((s) => { initial[s.id] = Object.fromEntries(secs.map((sec) => [sec, ''])) })
    setScores(initial)
    setPhase('grading')
  }
  ...
  const allScoresFilled = presentStudents.length > 0 && presentStudents.every((s) => sections.every((sec) => scores[s.id]?.[sec] !== '' && scores[s.id]?.[sec] !== undefined))

  const save = () => {
    if (saving) return
    const max = parseFloat(maxScore)
    // الرصد بيتم للطلبة الحاضرين بس (الغايبين مستثنيين من القائمة فوق)
          ...
          <div>
            ...
            {selectedGroups.length > 0 && (
              <p className='text-fg-subtle text-[11px] mt-1'>
                {isArabic
                  ? `سيتم رصد ${presentStudents.length} طالب${absentStudents.length > 0 ? ` (مستثنى ${absentStudents.length} غايبين من غير درجات)` : ''}`
                  : `${presentStudents.length} students will be graded${absentStudents.length > 0 ? ` (${absentStudents.length} absent excluded)` : ''}`}
              </p>
            )}
          </div>
        ...
        <div>
          ...
          {absentStudents.length > 0 && (
            <div className='mt-3 border-t border-subtle pt-2'>
              <p className='text-[11px] font-bold text-amber-400'>{t('exam_absent_students')}</p>
              <p className='text-[11px] text-fg-subtle'>{absentStudents.map((s) => s.name).join('، ')}</p>
              <p className='text-[10px] text-fg-subtle mt-0.5'>{t('exam_absent_note')}</p>
            </div>
          )}
        </div>
      )}
