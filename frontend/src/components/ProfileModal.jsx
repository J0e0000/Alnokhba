...
export default function ProfileModal({ open, student, allStudents, exams, dailyLogs, session, ranks, onClose, onEdit, onRemoveWarning, onEditPoints, attendancePct, paymentStatus, insights, qrMessageTemplate }) {
  ...
  useEffect(() => {
    ...
    import('qrcode').then(({ default: QRCode }) => Promise.all([
      QRCode.toDataURL(studentQRLink, { width: 240, margin: 4, color: { dark: '#0E2954', light: '#ffffff' } }),
      QRCode.toDataURL(studentQRLink, { width: 200, margin: 4, color: { dark: '#0E2954', light: '#ffffff' } }),
    ])).then(([main, card]) => {
