...
export default function ProfileModal({ open, student, allStudents, exams, dailyLogs, session, ranks, onClose, onEdit, onRemoveWarning, onEditPoints, attendancePct, paymentStatus, insights, qrMessageTemplate }) {
  ...
  useEffect(() => {
    ...
    // PERF: qrcode is loaded on demand — it used to be a static import,
    // forcing the whole library into the first bundle for every page.
    import('qrcode').then(({ default: QRCode }) => Promise.all([
      QRCode.toDataURL(studentQRLink, { width: 240, margin: 4, color: { dark: '#142D62', light: '#ffffff' } }),
      QRCode.toDataURL(studentQRLink, { width: 200, margin: 4, color: { dark: '#142D62', light: '#ffffff' } }),
    ])).then(([main, card]) => {
  ...
  const downloadQRCard = useCallback(async () => {
    ...
    try {
      ...
      // PERF: html2canvas-pro loads only when the user actually exports the
      // card (the old static import dragged it into the first bundle).
    ...
    <Modal open={open} onClose={onClose} title={t('student_file')} wide>
      ...
      <div className="bg-white rounded-xl p-4 space-y-4" style={{ colorScheme: 'light' }}>
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
            <h4 className="text-xl font-black text-[#142D62] flex items-center gap-2">
              {student.name}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-6 gap-2">
          <Stat label={t('points')} value={`${student.points} pt`} color="text-[#B86800]" />
          <Stat label={t('position')} value={`#${position}`} color="text-violet-600" />
          <Stat label={t('rank')} value={`🛡️ ${rank}`} color="text-[#B86800]" />
        ...
        {session && (session.lesson_topic || session.homework_text) && (
          <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
            <p className="text-sm font-bold text-[#B86800] mb-1">📚 {t('today_session')} ({student.group_name})</p>
                ...
                return (
                  <div key={ex.id} className="bg-slate-50 p-3 rounded-lg border border-slate-200">
                    <div className="flex justify-between text-sm mb-1">
                      <span className="font-bold text-[#B86800]">{ex.exam_title}</span>
                    ...
                    <div className="flex justify-between items-center text-xs">
                      ...
                      <span className="font-black text-[#B86800]">{pct}%</span>
                    </div>
                  </div>
                )
              })}
      ...
      <div className="fixed pointer-events-none" style={{ left: 0, top: 0, zIndex: -1, opacity: 0.01, maxWidth: '100vw' }}>
        <div ref={qrCardRef} className="bg-white p-6" style={{ width: 'min(420px, calc(100vw - 32px))', boxSizing: 'border-box', colorScheme: 'light' }}>
          <div className="border-2 border-[#142D62] rounded-2xl p-5 relative overflow-hidden" style={{ background: 'linear-gradient(180deg,#ffffff 0%,#F8FAFC 100%)' }}>
            <div className="absolute top-0 right-0 w-16 h-16 border-b-2 border-l-2 border-[#FB9C1B] rounded-bl-2xl" />
            <div className="absolute bottom-0 left-0 w-16 h-16 border-t-2 border-r-2 border-[#FB9C1B] rounded-tr-2xl" />
            <div className="flex flex-col items-center gap-1 pb-3 mb-3 border-b border-slate-200">
              ...
              <p className="font-black text-[#142D62] text-sm">{isArabic ? 'النخبة' : 'Al-Nokhba'}</p>
              <p className="text-[10px] text-slate-400">{isArabic ? 'إدارة الحصص الذكية' : 'Smart Class Management'}</p>
            </div>

            <div className="text-center mb-4">
              <p className="text-lg font-black text-[#142D62] leading-tight">{student.name}</p>
              <span className="inline-block bg-amber-50 text-[#B86800] text-[10px] font-bold px-2 py-0.5 rounded-full mt-1">{t('student_label')}</span>
            ...
            <div className="flex justify-center">
              <div className="border-2 border-[#FB9C1B] rounded-xl p-2 bg-white">
                {qrCardDataUrl && <img src={qrCardDataUrl} alt="QR" className="w-40 h-40" />}
              </div>
            </div>

            <p className="text-center text-[#142D62] text-[11px] font-bold mt-4 pt-3 border-t border-slate-200">
              {t('scan_to_attend')}
            </p>
