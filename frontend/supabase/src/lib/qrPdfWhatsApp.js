...
import { isValidPhone, getWhatsAppPhoneDigits } from './helpers'
 ...
 */
export async function getOrCreateStudentToken(studentId) {
  // ─── Path 1: RPC (preferred — bypasses RLS, atomic) ───
  try {
    const { data: rpcToken, error: rpcErr } = await supabase
      .rpc('get_or_create_student_qr_token', { p_student_id: studentId })

    if (!rpcErr && rpcToken) {
      return rpcToken
    }
    if (rpcErr) {
      console.warn('[getOrCreateStudentToken] RPC failed, trying direct path:', rpcErr.message || rpcErr)
    }
  } catch (e) {
    console.warn('[getOrCreateStudentToken] RPC threw, trying direct path:', e)
  }
  ...
  for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
    try {
      ...
      console.error(`[getOrCreateStudentToken] unexpected error (attempt ${attempt + 1}):`, err)
      await sleep(250 * (attempt + 1))
    }
  }

  // ─── All DB paths failed ───
  // SECURITY FIX (migration_019): NO stateless fallback. Return null and let
  // the caller surface the error to the user. A bare student_id is NOT a
  // valid token — see function docstring above.
 ...
 * @returns {string}
 */
function buildQRMessage(studentName, studentLink, template) {
 ...
 */
export function sendQRViaWhatsApp(phone, studentName, studentLink, template) {
  const cleanPhone = getWhatsAppPhoneDigits(phone)
  if (!cleanPhone || !isValidPhone(phone)) return false

  const message = buildQRMessage(studentName, studentLink, template)
  const waUrl = `https://wa.me/${cleanPhone}?text=${encodeURIComponent(message)}`
  const popup = window.open(waUrl, 'nokhba_whatsapp')
...
export function sendReportWhatsApp(phone, textReport) {
  ...
  const waUrl = `https://wa.me/${cleanPhone}?text=${encodeURIComponent(textReport)}`
  const popup = window.open(waUrl, 'nokhba_whatsapp')
  return popup ? waUrl : false
}
