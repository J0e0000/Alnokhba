...
await step('login', async () => {
  ...
  await page.waitForTimeout(1200)
  // Dismiss the first-login interactive tutorial tour (production feature)
...
await step('open session', async () => {
  ...
  if (await card.isVisible().catch(() => false)) { await card.click(); await page.waitForTimeout(1500) }
  else if (await btn.isVisible().catch(() => false)) { await btn.click(); await page.waitForTimeout(1200) }
  else console.log('    (no session card/button — may already be open)')
})
