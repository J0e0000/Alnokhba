-- Migration 032: student-scoped parent push subscriptions.
-- Uses the existing push tables; no new database or notification table is created.
...
create index if not exists idx_push_jobs_student
  on public.push_notification_jobs(student_id, processed_at);
...
begin
  ...
  if v_student_id is null and p_token ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$' then
  ...
  insert into public.push_subscriptions (teacher_id, student_id, endpoint, p256dh, auth, user_agent, updated_at)
  values (v_teacher_id, v_student_id, trim(p_endpoint), trim(p_p256dh), trim(p_auth), p_user_agent, now())
...
begin
  ...
  values(new.teacher_id, new.related_student_id, coalesce(new.title,'تنبيه جديد'), coalesce(new.body,'لديك تنبيه جديد'), '/', new.id);
  return new;
end;
$$;
